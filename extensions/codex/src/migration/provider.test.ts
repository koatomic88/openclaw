...
describe("buildCodexMigrationProvider", () => {
  ...
  it("imports Codex auth.json OAuth and seeds cached OpenAI Codex models", async () => {
    ...
    expect(authProfiles["openai-codex:account-acct_test"]).toEqual(
  ...
  it("does not collapse Codex OAuth accounts that share an email", async () => {
    ...
    expectRecordFields(findItem(result.items, "auth:openai-codex"), { status: "migrated" });
    const authProfiles = readFixtureAuthProfiles(fixture);
    expect(authProfiles["openai-codex:account-acct_old"]).toEqual(
      expect.objectContaining({
        access: "old-access-token",
        accountId: "acct_old",
        email: sharedEmail,
    ...
    expect(authProfiles["openai-codex:account-acct_new"]).toEqual(
      expect.objectContaining({
        access: accessToken,
        accountId: "acct_new",
        email: sharedEmail,
