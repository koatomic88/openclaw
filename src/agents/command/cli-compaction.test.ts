...
function createSessionState(params: {
  ...
  seedSqliteTranscript({ sessionId: params.sessionId, cwd: tmpDirForSessionState.current });
  ...
  upsertSessionEntry({ agentId: "main", sessionKey: params.sessionKey, entry: sessionEntry });
  return { sessionEntry, sessionStore };
}

const tmpDirForSessionState = {
  current: "",
};
...
describe("runCliTurnCompactionLifecycle", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDirForSessionState.current = tmpDir;
  ...
  it("compacts over-budget CLI transcripts and clears external CLI resume state", async () => {
    ...
    const { sessionEntry, sessionStore } = createSessionState({
      sessionId,
      sessionKey,
      entry: {
        cliSessionBindings: {
  ...
  it("routes OpenAI Codex harness CLI compaction through native harness compaction", async () => {
    ...
    const recordCliCompactionInSessionEntry = vi.fn(async () => ({
      ...sessionEntry,
      compactionCount: 1,
    }));
    setCliCompactionTestDeps({
    ...
    expect(recordCliCompactionInSessionEntry).toHaveBeenCalledWith(
      expect.objectContaining({ provider: "openai", sessionKey, tokensAfter: 100 }),
    );
    expect(updatedEntry?.compactionCount).toBe(1);
  });
  ...
  it("surfaces nonrecoverable native harness CLI compaction failures", async () => {
    ...
    const recordCliCompactionInSessionEntry = vi.fn();
    setCliCompactionTestDeps({
      resolveContextEngine: async () => buildContextEngine({ compactCalls }),
    ...
    expect(compactAgentHarnessSession).toHaveBeenCalledTimes(1);
    expect(compactCalls).toHaveLength(0);
    expect(recordCliCompactionInSessionEntry).not.toHaveBeenCalled();
  });
  ...
  it.each([
    {
      name: "pinned harness has no native compactor",
      sessionKey: "agent:main:external-harness",
      sessionId: "session-external-harness",
    ...
    {
      name: "Codex native binding is stale",
      sessionKey: "agent:main:codex-stale-binding",
      sessionId: "session-codex-stale-binding",
      provider: "codex",
      model: "gpt-5.5",
      agentHarnessId: "codex",
      nativeResult: {
    ...
    {
      name: "Codex native compaction returns a raw missing thread reason",
      sessionKey: "agent:main:codex-raw-stale-binding",
      sessionId: "session-codex-raw-stale-binding",
      provider: "codex",
      model: "gpt-5.5",
      agentHarnessId: "codex",
      nativeResult: {
  ...
  ])("falls back to context-engine compaction when $name", async (params) => {
    const { sessionEntry, sessionStore } = createSessionState({
      sessionId: params.sessionId,
      sessionKey: params.sessionKey,
      entry: { agentHarnessId: params.agentHarnessId },
    });
    const compactCalls: Array<Parameters<ContextEngine["compact"]>[0]> = [];
    const compactAgentHarnessSession = vi.fn(async () => params.nativeResult);
    const maintenance = vi.fn(async () => ({ changed: false, bytesFreed: 0, rewrittenEntries: 0 }));
    const recordCliCompactionInSessionEntry = vi.fn(async () => ({
      ...sessionEntry,
      compactionCount: 1,
